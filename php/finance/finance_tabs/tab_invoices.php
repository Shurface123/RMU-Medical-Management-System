...
$fee_list = [];
$fq = mysqli_query($conn, "SELECT fee_id,service_name,service_code,base_amount,tax_rate_pct,is_taxable FROM fee_schedule WHERE is_active=1 ORDER BY service_name");
if ($fq)
...
<div id="sec-invoices" class="dash-section">
  ...
  <div class="fin-filter-row">
    <div class="adm-search-wrap" style="min-width:220px;flex:2;">
      ...
      <input type="text" id="invSearch" class="adm-search-input" placeholder="Search invoice number, patient..."
        oninput="filterTable('invSearch','invoiceTable')">
    </div>
          ...
          <?php if (empty($inv_list)): ?>
            <tr>
              <td colspan="10" style="text-align:center;padding:4rem;color:var(--text-muted);">
                ...
                No invoices yet. <a href="#" onclick="openModal('modalCreateInvoice')"
                  style="color:var(--role-accent);">Create the first invoice</a>
              </td>
            </tr>
          <?php else:
            foreach ($inv_list as $inv):
              $badge_map = [
                ...
                'Paid' => 'badge-paid',
                'Overdue' => 'badge-overdue-fin',
                'Cancelled' => 'badge-cancelled',
                'Void' => 'badge-void',
                'Written Off' => 'badge-void'
              ];
              ...
              <tr class="<?= $tr_class ?>" data-status="<?= $inv['status'] ?>" data-date="<?= $inv['invoice_date'] ?>">
                ...
                <td style="color:<?= $inv['balance_due'] > 0 ? 'var(--danger)' : 'var(--success)'; ?>">
                  <strong><?= number_format($inv['balance_due'], 2) ?></strong></td>
    ...
    <div class="adm-modal-body">
      <form id="formCreateInvoice">
        ...
        <div class="adm-form-group">
          ...
          <select id="invPatientId" name="patient_id" class="adm-search-input" required
          ...
          <div id="patientInfoBox"
            style="margin-top:.8rem;padding:1rem;background:var(--surface-2);border-radius:10px;border:1px solid var(--border);display:none;font-size:1.2rem;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;">
          ...
          <div class="adm-form-group">
            ...
            <input type="date" name="due_date" class="adm-search-input"
        ...
        <div class="inv-summary">
          ...
          <div class="inv-summary-row"><span>Discount</span><span id="sumDiscount"
        ...
        <div class="adm-form-group" style="margin-top:1.5rem;">
          ...
          <textarea name="notes" class="adm-search-input" rows="2" style="resize:vertical;"
            placeholder="Optional invoice notes..."></textarea>
        </div>
      </form>
    </div>
...
<script>
  ...
  function addLineItem(data = {}) {
    ...
    const feeOpts = feeSchedule.map(f =>
      `<option value="${f.fee_id}" data-price="${f.base_amount}" data-tax="${f.tax_rate_pct}" data-taxable="${f.is_taxable}">${f.service_name} (GHS ${parseFloat(f.base_amount).toFixed(2)})</option>`
    ).join('');
