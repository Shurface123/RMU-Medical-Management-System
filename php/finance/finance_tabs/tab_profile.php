...
$settings_row = mysqli_fetch_assoc(mysqli_query($conn, "SELECT s.* FROM finance_settings s JOIN finance_staff fs ON s.finance_staff_id = fs.finance_staff_id WHERE fs.user_id = $user_id LIMIT 1")) ?: [];
...
<div id="sec-profile" class="dash-section">
  ...
  <div class="fin-hero" style="margin-bottom:2rem;">
    <div style="position:relative;">
      <?php if (!empty($prof['profile_image']) && file_exists(dirname(dirname(dirname(__DIR__))) . '/' . $prof['profile_image'])): ?>
        <img src="/RMU-Medical-Management-System/<?= htmlspecialchars($prof['profile_image']) ?>"
          style="width:90px;height:90px;border-radius:50%;object-fit:cover;border:3px solid rgba(255,255,255,.4);"
          alt="Profile">
      <?php else: ?>
        <div
          style="width:90px;height:90px;border-radius:50%;background:rgba(255,255,255,.2);display:flex;align-items:center;justify-content:center;font-size:3rem;border:3px solid rgba(255,255,255,.4);">
          <i class="fas fa-user-tie"></i>
        </div>
      <?php endif; ?>
      <label for="profilePhotoInput"
        style="position:absolute;bottom:0;right:0;width:28px;height:28px;border-radius:50%;background:var(--role-gold);color:#000;display:flex;align-items:center;justify-content:center;font-size:1.1rem;cursor:pointer;"
        title="Change Photo"><i class="fas fa-camera"></i></label>
    ...
    <div style="flex:1;position:relative;z-index:1;">
      ...
      <p><?= htmlspecialchars($prof['designation'] ?? ucfirst(str_replace('_', ' ', $user_role) ?: 'Finance Officer')) ?>
        &mdash; <?= htmlspecialchars($prof['department'] ?? 'Finance & Revenue') ?></p>
      <div style="margin-top:.6rem;">
        <span class="fin-hero-badge"><i
            class="fas fa-id-badge"></i><?= htmlspecialchars($prof['staff_code'] ?? 'FIN-000') ?></span>
        <span class="fin-hero-badge"><i class="fas fa-envelope"></i><?= htmlspecialchars($prof['email'] ?? '—') ?></span>
        <?php if (!empty($prof['last_login'])): ?><span class="fin-hero-badge"><i class="fas fa-clock"></i>Last login:
            <?= date('d M Y, g:i A', strtotime($prof['last_login'])) ?></span><?php endif; ?>
      </div>
      <!-- Completeness -->
      <div style="margin-top:1.2rem;">
        ...
        <div style="height:6px;background:rgba(255,255,255,.2);border-radius:10px;overflow:hidden;max-width:280px;">
          <div
            style="height:100%;width:<?= $complete_pct ?>%;background:linear-gradient(90deg,#fff,rgba(255,255,255,.6));border-radius:10px;transition:width .6s ease;">
          </div>
        </div>
      </div>
    </div>
    <div style="position:relative;z-index:1;">
      <div
        style="display:flex;align-items:center;gap:.6rem;background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.28);border-radius:50px;padding:.5rem 1.2rem;font-size:1.2rem;">
        <span style="width:10px;height:10px;border-radius:50%;background:#27AE60;box-shadow:0 0 6px #27AE60;"></span>
        Online
      </div>
    </div>
  </div>
            ...
            foreach ($fields as [$label, $name, $type, $val]): ?>
              <div class="adm-form-group">
                ...
                <input type="<?= $type ?>" name="<?= $name ?>" value="<?= htmlspecialchars($val) ?>"
                  class="adm-search-input profile-field" readonly>
                <?php if ($name === 'date_of_birth' && $age !== null): ?>
                  <div style="font-size:1.15rem;color:var(--text-muted);margin-top:.3rem;">Age: <?= $age ?> years</div>
                <?php endif; ?>
              </div>
            <?php endforeach; ?>
          </div>
  ...
  <div id="profProfessional" class="prof-section" style="display:none;">
    <div class="adm-card">
      ...
      <div class="adm-card-body">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:2rem;">
          <?php foreach ([
            ...
            <div class="adm-form-group">
              <label><?= $label ?></label>
              <div
                style="padding:1rem 1.2rem;background:var(--surface-2);border:1px solid var(--border);border-radius:10px;font-size:1.35rem;">
        ...
        <div
          style="margin-top:2rem;padding:1.5rem;background:var(--surface-2);border-radius:var(--radius-md);border:1px solid var(--border);">
          <div style="font-weight:700;font-size:1.4rem;margin-bottom:1.2rem;"><i class="fas fa-list-check"
              style="color:var(--role-accent);"></i> Profile Completeness</div>
          <?php foreach ($completeness_fields as $field => $done): ?>
            <div style="display:flex;align-items:center;gap:.8rem;padding:.5rem 0;">
              <i class="fas fa-<?= $done ? 'check-circle' : 'circle-exclamation' ?>"
                style="color:<?= $done ? 'var(--success)' : 'var(--warning)' ?>;font-size:1.4rem;"></i>
              <span
  ...
  <div id="profSecurity" class="prof-section" style="display:none;">
      ...
      <div class="adm-card-body">
        <form id="formChangePassword" style="max-width:480px;">
          ...
          <div class="adm-form-group">
            ...
            <input type="password" name="new_password" id="newPwdInput" class="adm-search-input" required
              oninput="checkPwdStrength(this.value)">
            <div style="margin-top:.5rem;height:6px;background:var(--border);border-radius:10px;overflow:hidden;">
              <div id="pwdStrengthBar" style="height:100%;width:0%;border-radius:10px;transition:all .3s;"></div>
            </div>
    ...
    <div class="adm-card" style="margin-top:2rem;">
      <div class="adm-card-header">
        <h3><i class="fas fa-shield-check"></i> Account Activity</h3>
      </div>
      <div class="adm-card-body">
        <?php
        $activity = [];
        ...
        <?php else:
          foreach ($activity as $act): ?>
            <div
              style="display:flex;gap:1rem;padding:.8rem 0;border-bottom:1px solid var(--border);align-items:flex-start;">
              <div
              ...
              <div style="flex:1;">
                <div style="font-size:1.3rem;font-weight:500;"><?= htmlspecialchars($act['action_type'] ?? 'Action') ?></div>
                <div style="font-size:1.15rem;color:var(--text-muted);"><?= htmlspecialchars($act['description'] ?? '') ?></div>
              </div>
              <div style="font-size:1.1rem;color:var(--text-muted);white-space:nowrap;">
                <?= date('d M, g:i A', strtotime($act['created_at'])) ?></div>
            </div>
      ...
      <div class="adm-card-body">
        <div id="notifPrefsForm">
          ...
          $notif_types = [
            ...
            ['notif_invoice_overdue', 'Invoice Overdue Alert'],
            ['notif_insurance_update', 'Insurance Claim Update'],
            ['notif_waiver_request', 'Waiver Request'],
            ['notif_refund_request', 'Refund Request'],
            ['notif_budget_alert', 'Budget Alert'],
            ['notif_reconciliation', 'Daily Reconciliation'],
            ['notif_system_alerts', 'System Announcements'],
          ];
          foreach ($notif_types as [$key, $label]):
            ...
            <div
              ...
              <label style="position:relative;display:inline-block;width:44px;height:24px;cursor:pointer;">
                <input type="checkbox" id="pref_<?= $key ?>" <?= $val ? 'checked' : '' ?>
                  onchange="saveNotifPref('<?= $key ?>',this.checked)" style="opacity:0;width:0;height:0;">
                <span id="toggle_<?= $key ?>"
                  style="position:absolute;inset:0;border-radius:24px;background:<?= $val ? 'var(--role-accent)' : 'var(--border)' ?>;transition:.3s;"><span
                    style="position:absolute;content:'';height:18px;width:18px;left:3px;bottom:3px;background:#fff;border-radius:50%;transition:.3s;transform:<?= $val ? 'translateX(20px)' : 'none' ?>;display:block;"></span></span>
              </label>
            </div>
          <?php endforeach; ?>
        </div>
