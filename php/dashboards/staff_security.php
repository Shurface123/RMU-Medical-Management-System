...
/* ─── Sanitize / Escape ───────────────────────────────────── */
function sanitize($v) { return htmlspecialchars(trim(stripslashes((string)$v)), ENT_QUOTES, 'UTF-8'); }
function e($v)        { return htmlspecialchars((string)$v, ENT_QUOTES, 'UTF-8'); }
function confirmAction($msg='Are you sure?') { return true; } // JS-side
...
function handleUpload($fileKey, $subDir, $allowedTypes=['jpg','jpeg','png','pdf','gif'], $maxMB=5) {
    ...
    $file = $_FILES[$fileKey];
    if ($file['size'] > $maxMB * 1024 * 1024) return ['error' => "File must be under {$maxMB}MB"];
    $ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
    if (!in_array($ext, $allowedTypes)) return ['error' => 'File type not allowed'];
...
function json_ok($msg='Success', $data=[]) {
    echo json_encode(['success'=>true,'message'=>$msg]+$data); exit();
}
