<?php
// ============================================================
// NURSE DASHBOARD - NOTIFICATIONS TAB
// ============================================================
if (!isset($conn)) exit;
...
<div class="tab-content <?= ($active_tab === 'notifications') ? 'active' : '' ?>" id="notifications">
    ...
    <div class="sec-header">
        <div>
            ...
            <p style="font-size:1.3rem; color:var(--text-muted);">Manage all your system alerts, clinical updates, and messages.</p>
        </div>
        <div style="display:flex; gap:1rem;">
            ...
            <?php endif; ?>
            <button class="btn btn-ghost" onclick="window.location.href='?tab=notifications';" style="border-radius:12px; font-weight:700;"><span class="btn-text">
    ...
    <!-- Filter Tabs -->
    <div class="adm-tab-group" style="margin-bottom:2rem; border-bottom:1px solid var(--border); padding-bottom:1rem; gap:1rem;">
        <button class="ftab active" id="btn-all-notifs" onclick="filterNotifs('all')">
            <i class="fas fa-inbox"></i> All Feed
        </button>
        <button class="ftab" id="btn-unread-notifs" onclick="filterNotifs('unread')">
            ...
            <?php else: ?>
                <div style="display:flex; flex-direction:column;">
                    ...
                    foreach ($notifs as $n): 
                        ...
                        if (str_contains($nType, 'emergency') || str_contains($nType, 'critical')) {
                            $nIcon = 'fa-ambulance';
                            $nColor = 'var(--danger)';
                            $bgLight = 'rgba(231,76,60,0.1)';
                        } elseif (str_contains($nType, 'ward') || str_contains($nType, 'transfer')) {
                            $nIcon = 'fa-bed';
                            $nColor = 'var(--info)';
                            $bgLight = 'rgba(52,152,219,0.1)';
                        } elseif (str_contains($nType, 'med') || str_contains($nType, 'pharm')) {
                            $nIcon = 'fa-pills';
                            $nColor = 'var(--success)';
                            $bgLight = 'rgba(46,204,113,0.1)';
                    ...
                    ?>
                        <div class="notif-item" data-read="<?= $is_read ? 'true' : 'false' ?>" 
                             style="display:flex; align-items:flex-start; gap:1.5rem; padding:1.8rem 2rem; border-bottom:1px solid var(--border); background:<?= $is_read ? 'transparent' : 'rgba(var(--primary-rgb),0.02)' ?>; transition:all 0.2s;">
                            ...
                            <div style="flex:1;">
                                ...
                                <div style="display:flex; justify-content:space-between; align-items:center; margin-top:.5rem;">
                                    <span class="adm-badge" style="background:var(--surface-2); color:var(--text-secondary); border:none; padding:.3rem .8rem; font-size:1rem; text-transform:uppercase;">
...
function markNotificationRead(id) {
    if (!id) return;
    $.post('../nurse/process_notifications.php', {
        action: 'mark_read',
        notification_id: id,
...
function markAllNotificationsRead() {
    Swal.fire({
        ...
        text: 'This will clear all your active notifications.',
        icon: 'question',
