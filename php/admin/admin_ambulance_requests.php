                    ...
                    <td>
                        <div style="display:flex; gap:0.5rem; flex-wrap:wrap;">
                            <?php if($r['status'] === 'Pending'): ?>
                                <form method="POST" style="margin:0;"><input type="hidden" name="action" value="dispatch"><input type="hidden" name="req_id" value="<?= $r['id'] ?>"><button title="Mark Dispatched" class="adm-btn adm-btn-primary adm-btn-sm" style="padding:6px; font-size:12px; background:var(--warning); border-color:var(--warning);"><i class="fas fa-truck-fast"></i> Dispatch</button></form>
                                <form method="POST" style="margin:0;"><input type="hidden" name="action" value="cancel"><input type="hidden" name="req_id" value="<?= $r['id'] ?>"><button title="Cancel Request" class="adm-btn adm-btn-outline adm-btn-sm" style="padding:6px; font-size:12px; border-color:var(--danger); color:var(--danger);"><i class="fas fa-times"></i></button></form>
                            <?php elseif($r['status'] === 'Dispatched' || $r['status'] === 'In Transit'): ?>
                                <form method="POST" style="margin:0;"><input type="hidden" name="action" value="complete"><input type="hidden" name="req_id" value="<?= $r['id'] ?>"><button title="Mark Completed" class="adm-btn adm-btn-success adm-btn-sm" style="padding:6px; font-size:12px;"><i class="fas fa-check"></i> Complete</button></form>
                            <?php else: ?>
                                <form method="POST" style="margin:0;" onsubmit="return confirm('Delete this record permanently?');"><input type="hidden" name="action" value="delete"><input type="hidden" name="req_id" value="<?= $r['id'] ?>"><button title="Delete Record" class="adm-btn adm-btn-ghost adm-btn-sm" style="padding:6px; font-size:12px; color:var(--text-muted);"><i class="fas fa-trash"></i></button></form>
                            <?php endif; ?>
                        </div>
                    </td>
                </tr>
