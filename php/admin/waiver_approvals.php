...
$q_pending = mysqli_query($conn, "
    SELECT w.*, u.name as patient_name, u.email as patient_email,
...
$q_recent = mysqli_query($conn, "
    SELECT w.*, u.name as patient_name, a.name as admin_name
    FROM payment_waivers w
    JOIN patients p ON w.patient_id = p.id
    JOIN users u ON p.user_id = u.id
    LEFT JOIN users a ON w.approved_by = a.id
    WHERE w.status IN ('Approved', 'Rejected')
    ORDER BY w.approved_at DESC LIMIT 50
");
if ($q_recent) while ($row = mysqli_fetch_assoc($q_recent)) $recent[] = $row;

$approvedTotal = array_sum(array_column(array_filter($recent, fn($r) => $r['status'] === 'Approved'), 'waived_amount'));
?>
...
<style>
:root { --primary:#10b981; --primary-light:rgba(16,185,129,.15); --danger:#ef4444; --warning:#f59e0b; --info:#3b82f6; }
.staff-hero{display:flex;align-items:center;gap:2rem;padding:2rem 2.5rem;margin-bottom:2.5rem;background:linear-gradient(135deg,#10b981,#065f46);border-radius:var(--radius-lg);color:#fff;box-shadow:var(--shadow-md);flex-wrap:wrap;position:relative;overflow:hidden;}
...
.stat-mini-val.danger{color:var(--danger);} .stat-mini-val.info{color:var(--info);}
...
.btn-success{background:var(--success,#10b981);color:#fff;} .btn-success:hover{opacity:.88;transform:translateY(-1px);}
.btn-danger{background:var(--danger);color:#fff;} .btn-danger:hover{opacity:.88;transform:translateY(-1px);}
...
.modal-header h3{margin:0;font-size:1.4rem;color:var(--text-primary);display:flex;align-items:center;gap:.5rem;}
...
.form-control{width:100%;padding:1rem 1.3rem;border:1.5px solid var(--border);border-radius:var(--radius-sm);background:var(--surface);color:var(--text-primary);font-family:'Poppins',sans-serif;font-size:1.15rem;transition:var(--transition);outline:none;box-sizing:border-box;resize:vertical;min-height:80px;}
...
.toast-msg.toast-danger{border-left-color:var(--danger);} .toast-msg.toast-success{border-left-color:var(--success,#10b981);}
...
<main class="adm-main">
    ...
    <div class="adm-content" style="animation:fadePop .35s ease;">
        <div class="staff-hero">
            <i class="fas fa-receipt hero-bg-icon"></i>
            <div class="staff-hero-avatar"><i class="fas fa-hand-holding-usd"></i></div>
            <div class="staff-hero-info">
                <h2>Patient Waiver Approvals</h2>
                <p>Review and authorize financial waivers and discounts for eligible patients.</p>
            </div>
        </div>

        <div class="stat-grid">
            <div class="stat-mini">
                <div class="stat-mini-icon" style="color:var(--warning);background:rgba(245,158,11,.15);"><i class="fas fa-clock"></i></div>
                <div class="stat-mini-val" style="color:var(--warning);"><?= count($pending) ?></div>
                <div class="stat-mini-lbl">Pending Review</div>
            </div>
            <div class="stat-mini">
                <div class="stat-mini-icon" style="color:var(--primary);background:var(--primary-light);"><i class="fas fa-check-circle"></i></div>
                <div class="stat-mini-val"><?= count(array_filter($recent, fn($r) => $r['status'] === 'Approved')) ?></div>
            ...
            <div class="stat-mini">
                ...
                <div class="stat-mini-val info">GHS <?= number_format($approvedTotal, 0) ?></div>
                <div class="stat-mini-lbl">Total Waived</div>
            </div>
        </div>

        <div class="card">
            <div class="card-header">
                <h3><i class="fas fa-exclamation-circle" style="color:var(--warning);"></i> Pending Waiver Requests</h3>
            </div>
            <?php if (empty($pending)): ?>
                <div style="text-align:center;padding:5rem;color:var(--text-muted);">
                    <i class="fas fa-check-circle" style="font-size:4rem;color:var(--primary);margin-bottom:1rem;display:block;"></i>
                    <h3 style="color:var(--text-primary); margin-bottom:.5rem;">All Clear!</h3>
                    <p style="font-size:1.1rem;">No new waiver requests pending approval.</p>
                </div>
            <?php else: ?>
            <div style="overflow-x:auto;">
                <table class="stf-table" id="pendingTable">
                    <thead><tr><th>Waiver # / Invoice</th><th>Patient</th><th>Type</th><th>Amount (GHS)</th><th>Reason</th><th style="text-align:right;">Actions</th></tr></thead>
                    <tbody>
                        <?php foreach ($pending as $p): ?>
                        <tr id="row_<?= $p['waiver_id'] ?>">
                            <td>
                                <strong style="color:var(--primary);font-size:1.1rem;"><?= htmlspecialchars($p['waiver_number']) ?></strong>
                                <div style="font-size:.9rem;color:var(--text-muted);margin-top:.2rem;">Inv: <?= htmlspecialchars($p['invoice_number'] ?? 'N/A') ?></div>
                            </td>
                            <td><strong style="font-size:1.1rem;"><?= htmlspecialchars($p['patient_name']) ?></strong></td>
                            <td><span style="background:rgba(59,130,246,.15);color:var(--info);padding:.3rem .8rem;border-radius:12px;font-size:.95rem;font-weight:700;border:1px solid rgba(59,130,246,.3);"><?= htmlspecialchars($p['waiver_type']) ?></span></td>
                            <td>
                                <div style="font-size:1.3rem;font-weight:800;color:var(--primary);">GHS <?= number_format($p['waived_amount'], 2) ?></div>
                                <div style="font-size:.9rem;color:var(--text-muted);">of <?= number_format($p['original_amount'], 2) ?></div>
                            </td>
                            <td style="max-width:220px;color:var(--text-secondary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="<?= htmlspecialchars($p['reason']) ?>"><?= htmlspecialchars(substr($p['reason'], 0, 60)) ?>...</td>
                            <td style="text-align:right;">
                                <div style="display:inline-flex;gap:.5rem;">
                                    <button class="btn btn-success" style="padding:.6rem 1rem;" onclick="approveWaiver(<?= $p['waiver_id'] ?>)"><i class="fas fa-check"></i> Approve</button>
                                    <button class="btn btn-danger" style="padding:.6rem 1rem;" onclick="openRejectModal(<?= $p['waiver_id'] ?>)"><i class="fas fa-times"></i> Reject</button>
                                </div>
                            </td>
                        </tr>
                        <?php endforeach; ?>
        ...
        <div class="card">
            <div class="card-header"><h3><i class="fas fa-history" style="color:var(--text-muted);"></i> Recent Waiver Actions</h3></div>
            <?php if (empty($recent)): ?>
                <div style="padding:3rem;text-align:center;color:var(--text-muted);">No recent actions.</div>
            <?php else: ?>
            <div style="overflow-x:auto;">
                <table class="stf-table" id="recentTable">
                    <thead><tr><th>Waiver #</th><th>Patient</th><th>Waived (GHS)</th><th>Status</th><th>Actioned By</th><th>Date</th></tr></thead>
                    <tbody>
                        <?php foreach ($recent as $r): ?>
                        <tr>
                            <td><strong style="color:var(--text-secondary);"><?= htmlspecialchars($r['waiver_number']) ?></strong></td>
                            <td><?= htmlspecialchars($r['patient_name']) ?></td>
                            <td style="font-weight:700;"><?= number_format($r['waived_amount'], 2) ?></td>
                            <td>
                                <?php if ($r['status'] === 'Approved'): ?>
                                    <span style="background:rgba(16,185,129,.15);color:var(--primary);padding:.3rem .8rem;border-radius:12px;font-weight:700;font-size:.95rem;border:1px solid rgba(16,185,129,.3);">Approved</span>
                                <?php else: ?>
                                    <span style="background:rgba(239,68,68,.15);color:var(--danger);padding:.3rem .8rem;border-radius:12px;font-weight:700;font-size:.95rem;border:1px solid rgba(239,68,68,.3);" title="<?= htmlspecialchars($r['rejection_reason'] ?? '') ?>">Rejected</span>
                                <?php endif; ?>
                            </td>
                            <td><?= htmlspecialchars($r['admin_name'] ?? 'System') ?></td>
                            <td style="color:var(--text-muted);"><?= date('d M Y, g:i A', strtotime($r['approved_at'])) ?></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
...
<div class="modal-bg" id="rejectModal">
    <div class="modal-box">
        <div class="modal-header">
            <h3 style="color:var(--danger);"><i class="fas fa-times-circle"></i> Reject Waiver</h3>
            <button class="modal-close" onclick="closeRejectModal()"><i class="fas fa-times"></i></button>
        </div>
        <div class="modal-body">
            <p style="font-size:1.1rem;color:var(--text-secondary);margin-bottom:1rem;">Please provide a reason for rejecting this waiver request.</p>
            <textarea id="rejectReason" class="form-control" rows="3" placeholder="Reason for rejection..."></textarea>
            <input type="hidden" id="rejectWaiverId">
        </div>
        <div class="modal-footer">
            <button class="btn btn-ghost" onclick="closeRejectModal()">Cancel</button>
            <button class="btn btn-danger" onclick="submitRejectWaiver()"><i class="fas fa-ban"></i> Confirm Rejection</button>
        </div>
    </div>
</div>
...
$(document).ready(function(){
    if($('#recentTable').length) {
        $('#recentTable').DataTable({responsive:true,pageLength:10,language:{search:'',searchPlaceholder:'Search history...'}});
...
async function approveWaiver(id){
    if(!confirm('Approve this waiver? It will reflect immediately in the patient\'s invoice balance.')) return;
    const fd=new FormData(); fd.append('action','approve_waiver'); fd.append('waiver_id',id);
    try{const res=await fetch('waiver_actions.php',{method:'POST',body:fd});const data=await res.json();
    if(data.success){showToast(data.message,'success');setTimeout(()=>location.reload(),1000);}else showToast(data.message||'Error','danger');}
    catch(e){showToast('Network error','danger');}
}
function openRejectModal(id){document.getElementById('rejectWaiverId').value=id;document.getElementById('rejectReason').value='';document.getElementById('rejectModal').classList.add('active');}
function closeRejectModal(){document.getElementById('rejectModal').classList.remove('active');}
async function submitRejectWaiver(){
    const id=document.getElementById('rejectWaiverId').value;
    ...
    try{const res=await fetch('waiver_actions.php',{method:'POST',body:fd});const data=await res.json();
    if(data.success){showToast(data.message,'success');closeRejectModal();setTimeout(()=>location.reload(),1000);}else showToast(data.message||'Error','danger');}
    catch(e){showToast('Network error','danger');}
}
</script>
