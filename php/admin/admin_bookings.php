...
");
if ($q_bk) {
    while($r = mysqli_fetch_assoc($q_bk)) {
        $bookings[] = $r;
    }
}
...
<main class="adm-main">
    <div class="adm-topbar">
        <div class="adm-topbar-left">
            ...
            <span class="adm-page-title"><i class="fas fa-calendar-check"></i> Public Bookings</span>
        </div>
    ...
    <div class="adm-content" style="animation:fadePop .35s ease;">
        <div class="staff-hero">
            ...
            <div class="staff-hero-avatar"><i class="fas fa-calendar-check"></i></div>
            <div class="staff-hero-info">
                <h2>Web Appointment Bookings</h2>
                <p>Manage and track all appointment requests submitted securely via the public landing page.</p>
            </div>
        ...
        <div class="card">
            <div class="card-header">
                <h3><i class="fas fa-clipboard-list" style="color:var(--primary);"></i> Booking Requests</h3>
            </div>
            <div class="card-body" style="padding:1rem;">
                ...
                <table class="stf-table" id="bookingsTable">
                    <thead>
                        <tr>
                            <th>Ref ID</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <div><strong style="color:var(--primary); font-size:1.2rem;">BK-<?= str_pad($b['booking_id'],5,'0',STR_PAD_LEFT) ?></strong></div>
                                <div style="font-size:0.9rem; color:var(--text-muted); margin-top:0.3rem;"><i class="fas fa-clock"></i> <?= date('M d, y', strtotime($b['created_at'])) ?></div>
                            </td>
                            <td>
                                <div style="font-weight:700; font-size:1.15rem; color:var(--text-primary);"><?= htmlspecialchars($b['patient_name']) ?></div>
                                <div style="font-size:1rem; color:var(--text-secondary); margin-top:0.2rem;"><i class="fas fa-phone" style="font-size:0.9rem;"></i> <?= htmlspecialchars($b['patient_phone']) ?></div>
                            </td>
                            <td>
                                <div><strong style="color:var(--text-primary);">Dr. <?= htmlspecialchars($b['doc_name']) ?></strong></div>
                                <div style="font-size:0.9rem; color:var(--text-muted); margin-top:0.2rem; background:var(--surface-2); display:inline-block; padding:0.1rem 0.5rem; border-radius:4px;"><?= htmlspecialchars($b['service_name']) ?></div>
                            </td>
                            <td>
                                <div style="font-weight:700; color:var(--text-primary);"><i class="far fa-calendar" style="color:var(--primary);"></i> <?= date('D, M d Y', strtotime($b['preferred_date'])) ?></div>
                                <div style="color:var(--text-secondary); margin-top:0.2rem;"><i class="far fa-clock" style="color:var(--warning);"></i> <?= date('h:i A', strtotime($b['preferred_time'])) ?></div>
                            </td>
                            <td>
                                <?php 
                                    $sClass = 'badge-warning';
                                    if ($b['status'] === 'confirmed') $sClass = 'badge-success';
                                    elseif ($b['status'] === 'cancelled') $sClass = 'badge-danger';
                                ?>
                                <span class="<?= $sClass ?>"><?= htmlspecialchars($b['status']) ?></span>
                                <span style="display:none;"><?= htmlspecialchars($b['status']) ?></span> <!-- For DataTables filter -->
                            </td>
                                ...
                                <div style="display:inline-flex; gap:0.5rem;">
                                    <?php if($b['status'] === 'pending'): ?>
                                        <form method="POST" style="margin:0;">
                                            ...
                                            <button title="Confirm Request" class="btn btn-success" style="padding:0.6rem 1rem;"><i class="fas fa-check"></i></button>
                                        </form>
                                        <form method="POST" style="margin:0;">
                                    ...
                                    <?php else: ?>
                                        <button title="Delete Record Permanently" class="btn btn-ghost" style="padding:0.6rem 1rem; color:var(--danger);" onclick="openDeleteModal(<?= $b['booking_id'] ?>)"><i class="fas fa-trash"></i></button>
                                    <?php endif; ?>
                                </div>
                            </td>
                        </tr>
...
<div class="modal-bg" id="deleteModal">
    <div class="modal-box">
        ...
        <div class="modal-body">
            <p style="font-size:1.15rem; color:var(--text-secondary); margin-bottom:1rem;">Are you sure you want to permanently delete this booking record? This action cannot be undone.</p>
        </div>
    ...
    $(document).ready(function() {
        if ($('#bookingsTable').length) {
            ...
            $('.ftab').on('click', function() {
                ...
                bookingsTable.column(4).search(filterVal ? '^.*' + filterVal + '.*$' : '', true, false).draw();
            });
        }
    });
